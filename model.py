...
class R_FSRCNN(object):
  ...
  def __init__(self, sess, config):
    ...
    self.pic_index = config.pic_index
  ...
  def build_model(self):
    ...
    self.W = tf.Variable(np.full(fill_value=1.0 / m , shape=[m], dtype=np.float32), name="LayerWeights")
    self.W_sum = tf.reduce_sum(self.W)
  ...
  def run_test(self):
    ...
    original = merge(test_label, [nx, ny])
    original = original.squeeze()

    psnr = skimage.measure.compare_psnr(original, result)
    ssim = skimage.measure.compare_ssim(original, result)
    print("R_FSRCNN:     PSNR = [%.7f]   SSIM = [%.7f]" % (psnr, ssim))
  ...
  def model(self):
    ...
    m = self.model_params[2]
    prev_layer = m * [None]
    prev_layer[0] = self.prelu(tf.nn.conv2d(conv_shrink, self.weights['w{}'.format(3)], strides=[1,1,1,1], padding='SAME') + self.biases['b{}'.format(3)], 3)
    sum_prev_layer = m * [None]
    for i in range(1, m):
      weights, biases = self.weights['w{}'.format(i+3)], self.biases['b{}'.format(i+3)]
      prev_layer[i] = self.prelu(tf.nn.conv2d(prev_layer[i-1], weights, strides=[1,1,1,1], padding='SAME') + biases, i+3)
    for i in range(0, m):
      sum_prev_layer[i] = prev_layer[i] * self.W[i] / self.W_sum
    sum_layer = tf.add_n(sum_prev_layer)
    ...
    conv_expand = self.prelu(tf.nn.conv2d(sum_layer, expand_weights, strides=[1, 1, 1, 1], padding='SAME') + expand_biases, 7)
  ...
  def save(self, checkpoint_dir, step):
    model_name = "R_FSRCNN.model"
    model_dir = "%s_%s" % ("R_fsrcnn", self.label_size)
